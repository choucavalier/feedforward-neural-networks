...
\begin{frame}
  \begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=2.5cm]
    \tikzstyle{every pin edge}=[<-,shorten <=1pt]
    \tikzstyle{neuron}=[circle,draw,minimum size=17pt,inner sep=0pt]
    \node[neuron, pin=left:$x$] (I) at (0,0) {};
    \node[neuron] (H) [right of=I] {};
    \node[neuron,pin={[pin edge={->}]right:$y$}, right of=H] (O) {};
    \path (I) edge (H);
    \path (H) edge (O);
  \end{tikzpicture}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Partial derivative of composed functions
...
\begin{frame}
  ...
  \begin{equation*}
    \begin{split}
      \frac{(f \circ g \circ h)(x, y)}{\partial x}& = \frac{\partial f}{\partial g} \cdot
